...
with open('C:\\Users\\User\\Desktop\\DS4A_workspace\\Final Project\\DS4A-Final_Project-Team_28\\Personal\\Camilo\\additional_stopwords.txt', 'rb') as f:
    sw_additional = f.read().decode('utf-8').replace(u'\r', u'').split(u'\n')

titles_2_clustering = [' '.join(w for w in p.split() if w not in sw_additional) for p in titles_2_clean]            

# Geografía
df_geoCol = pd.read_csv('C:\\Users\\User\\Desktop\\DS4A_workspace\\Final Project\\DS4A-Final_Project-Team_28\\Personal\\Camilo\\geografiaCol.txt',
                 delimiter = '\t',
                 header=None)
...
titles_2_clustering_f = [' '.join(w for w in p.split() if w not in geo_names) for p in titles_2_clustering]

#corpus = clean_text.Text.copy()
corpus = titles_2_clustering_f.copy()
...
LABEL_COLOR_MAP = {0:'r', 1: 'g', 2: 'b', 3: 'y', 4: 'peru', 5:'coral', 6:'olive',7:'cyan'}
...
#plot graph
colors = np.array(['red','green','blue','yellow','peru','coral','olive','cyan'])
...
documents_clustered = pd.DataFrame(titles_2_clustering_f, X_clustered, columns = ['titles'])
...
wm = Wordmesh(documents_grouped['titles'][0])
